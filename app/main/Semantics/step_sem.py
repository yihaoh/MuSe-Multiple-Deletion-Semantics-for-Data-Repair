...
class StepSemantics(AbsSemantics):
    ...
    def find_mss(self, schema):
        ...
        order = {t[0]: 1 for t in self.prov_graph.nodes}
        alias = {}
        for t in self.prov_graph.nodes:
            alias[t] = (t[0] + str(order[t[0]]), t[1])
            order[t[0]] += 1

        edgeList = []
        for t in self.prov_graph.edges:
            edgeList.append([alias[t[0]], alias[t[1]]])
        # print graph
        # print(self.prov_graph.nodes)
        # print(self.prov_graph.edges)
        ...
        realMSS = set()
        for t in mss:
            realMSS.add((t[0], (alias[t][0],) + t[1] + (self.prov_graph.nodes[t]['rule'],)))
            stat[t[0]] += 1

        # print(realMSS)
        return realMSS, edgeList, [stat['author'], stat['publication'], stat['writes'], stat['organization'], stat['cite']]
            ...
            for i in range(len(self.rules)):
                ...
                for assignment in cur_assignments:
                    if assignment not in assignments:
                        assignments.append([assignment, i + 1])
    ...
    def gen_prov_graph(self, assignments):
        ...
        for assign in assignments:
            ...
            self.prov_graph.add_edges_from([(t, assign[0][0]) for t in assign[0][1:]])
            for t in assign[0][1:]:
                self.prov_graph.node[t]['rule'] = assign[1]
            self.prov_graph.node[assign[0][0]]['rule'] = assign[1]
    ...
    def traverse_by_layer(self):
        ...
        mss = []   # was set before, the MSS according to the heuristic algorithm
        ...
        for ly in layers[1:]:
            ...
            if arg_max is not None:
                mss.append(arg_max) # was add before
        return mss
